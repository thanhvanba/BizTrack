...
const OrderFormData = ({ mode = 'create', order: orderProp, selectedProducts: selectedProductsProps, onSave, onChange }) => {
    ...
    const fetchOrderDetails = async () => {
        try {
            ...
            if (orderRes) {
                ...
                {
                    mode === 'return' && setReturnOrderData({
                        ...returnOrderData,
        ...
        const data = {
            ...
            return_details: orderEligibility?.products
                ...
                .map(item => ({
                    ...
                    refund_amount:
                        (item.product_return_price ?? item.product_retail_price)
                }))
        };
        try {
    ...
    const calculateDiscountAmount = () => {
        ...
        return discountProduct + parseFloat(mode === "return" ? returnOrderData.order_amount : orderDiscount);
    };
                    ...
                    render: (_, record) => (
                        <div className="flex items-center gap-1">
                            <InputNumber
                                ...
                                addonAfter="₫"
                                onChange={(value) => updatePriceReturn(record.product_id, value)}
                                formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                                parser={(value) => value.replace(/\$\s?|(,*)/g, "")}
                                className="w-32"
                            />
                            <Tooltip title={`Giá bán gốc: ${record.product_retail_price?.toLocaleString()} ₫`}>
                ...
                <div className="grid grid-cols-3 gap-4 mb-10 pb-10">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    >
                                        <Select
                                            ...
                                            disabled={mode !== 'create' && selectedProducts.length > 0}
                                        >
                                    ...
                                    {mode === 'return' &&
                                        <div>
                                            <span className="font-medium">Kho:</span>  {
                                                warehouses?.find(w => w.warehouse_id === form.getFieldValue("warehouse_id"))?.warehouse_name
                                                || "Không rõ kho hàng"
                                            }
                                        </div>
                                    }
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <label className="font-medium flex items-center gap-1">
                                            ...
                                            {form?.getFieldValue("order_amount") != null && (
                                                <Tooltip title={`Giảm giá đơn hàng: ${form?.getFieldValue("shipping_fee").toLocaleString()} ₫`}>
                                                    <InfoCircleOutlined style={{ color: "#1890ff" }} />
                                                </Tooltip>
                                            )}
                                        </label>
                    ...
                    <div className="p-4 bg-white rounded-lg shadow">
                        ...
                        <div>
                            ...
                            >
                                <Input variant="filled" placeholder="Số nhà, tên đường..." disabled={mode === 'return'} />
                            </Form.Item>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                >
                                    <Select placeholder="Chọn phương thức" variant="filled" disabled={mode === 'return'}>
                            ...
                            <Form.Item name="note" label="Ghi chú">
                                <TextArea placeholder="Ghi chú" variant="filled" disabled={mode === 'return'} rows={2} />
                            </Form.Item>
                        </div>
                        <Divider />
