...
export default function FinanceReport({ data = null, reportTitle = "Báo cáo kết quả hoạt động kinh doanh" }) {
    ...
    const processData = (rawData) => {
        ...
        const sumAll = (arr) => normalize(arr).reduce((sum, val) => sum + (val || 0), 0);
        const summary = {
    ...
    const getColumns = () => {
        ...
        const timeColumns = data.time_periods.map((period, index) => ({
            ...
            render: (values) => {
                ...
                const isSpecial = ['return_value', 'gross_profit', 'net_profit'].includes(dataSource.find(row => row.values === values)?.key);
    ...
    const exportPDF = () => {
        ...
        const tableBody = dataSource.map(row => {
            ...
            const total = formatCurrency(row.values?.reduce((sum, val) => sum + (val || 0), 0) || 0);
        ...
        <div>
            ...
            <div style={{
                ...
                <div>
                    ...
                    <p style={{ margin: '4px 0 0 0', color: '#6b7280' }}>Chi nhánh trung tâm</p>
                </div>
                ...
                <div style={{ overflowX: 'auto', width: '100%' }}>
                    <Table
                        ...
                        rowClassName={(record, index) => {
                            if (record.key === 'net_revenue' || record.key === 'gross_profit' || record.key === 'net_profit') {
